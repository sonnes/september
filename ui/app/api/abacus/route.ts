...
function generateQuestion(digits: number, numberCount: number): AbacusQuestion {
  let numbers: number[] = [];
  let sum = 0;
  let attempts = 0;
...
export async function POST(req: Request) {
  try {
    const body = (await req.json()) as AbacusRequest;
    ...
    const questions: AbacusQuestion[] = Array(questionCount)
      .fill(null)
      .map(() => {
    ...
    const dictation = questions.map((q, i) => ({
      ...q,
      audio: audioResults[i],
    }));

    const response: AbacusResponse = {
      digits,
      speed,
      dictation,
    };
