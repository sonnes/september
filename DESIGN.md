...
| 2026-08-21 | Desktop onboarding moves to a left indigo sidebar        | Applies to `apps/desktop` only; the web app keeps the hero header. The brand, the setup title, and the vertical step list sit in a fixed 288px indigo sidebar. The sidebar and the step surface are both inset cards on the `zinc-100` page, with the same radius, shadow, and 8px gutter. This returns the desktop app to a left step-rail and reverses the 2026-06-27 row for that app. Every section on a step stays open: the desktop onboarding has no collapsible groups. Controls come from shadcn primitives in `apps/desktop/src/components/ui/`. User-approved. |
