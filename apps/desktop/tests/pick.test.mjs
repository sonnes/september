...
const MODELS = [
  { id: "qwen/qwen3-next-80b-a3b-instruct:free", name: "Qwen: Qwen3 Next 80B (free)", free: true },
...
test("the chosen model stays, or the closed picker loses its name", () => {
  ...
  assert.deepEqual(kept.map((model) => model.id), [
    "qwen/qwen3-next-80b-a3b-instruct:free",
    "openai/gpt-5",
  ]);
  // A row the words already found is not a second row.
  assert.deepEqual(
    searchModels(MODELS, "qwen", "qwen/qwen3-next-80b-a3b-instruct:free").length,
    1,
  );
});
...
test("one list picks a model and a voice, and a dwell needs no hold", async () => {
  const list = await readText("src/blocks/pick-list.tsx");
  ...
  // Two columns of 44px rows, and one column in the narrow card of the rail.
  // A dropdown closes when a dwell moves away.
  assert.match(list, /sm:grid-cols-2/);
  assert.match(list, /columns = 2/);
  assert.match(card, /columns=\{1\}/);
  assert.match(list, /min-h-11/);
  assert.match(list, /matchesWords\(/);
  ...
  for (const [name, page] of [
    ...
    assert.match(page, /<PickList/, name);
    assert.doesNotMatch(page, /<Select[ >]/, name);
  }
  assert.match(settings, /searchModels\(/);
  // A user who needs no card must see which rows want one.
  assert.match(settings, /Paid/);
});
