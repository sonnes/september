...
import { describeSpace, hasWritingService } from "@/services/ai";
import { seedPhrases } from "@/services/phrase-sync";
import { appendToNote } from "@/rules/notes";
import { newSpaceDraft, rememberDraft } from "@/services/os";
import { Screen, ScreenHeader } from "@/blocks/screen";
import {
  createSteps,
  filterSpaces,
  freeTitle,
...
import { Composer, Problem, openParams, talkParams } from "@/blocks/space";
// -------------------------------------------------------------- new space
...
export function NewSpaceScreen() {
  ...
  // reached SQLite must patch that space, never make a second one beside it.
  ...
  const make = async (said: string) => {
    ...
    try {
      ...
      // The words of the user reach SQLite before any model answers, so a
   ...
   *
   * The words are in SQLite before the first model runs, so a space that
