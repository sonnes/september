...
const shareCardSvg = await satori(
  row(
    [
      row(
        [
          row([keycap(76), row(WORDMARK.map(([part, color]) =>
      ...
      row(
        TAGLINE.map((line) =>
          row(
            line.map(([part, color]) =>
              text(part, { color, fontFamily: 'Noto Sans', fontSize: 78, fontWeight: 700, letterSpacing: -2 })
            ),
            { gap: WORD_GAP }
          )
        ),
...
await sharp(Buffer.from(shareCardSvg)).png().toFile(`${publicDir}/og.png`);
