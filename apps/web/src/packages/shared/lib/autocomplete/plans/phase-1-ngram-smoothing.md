...
├── persistence.ts                 NEW — thin wrapper over @/packages/shared/lib/indexeddb
...
>
> **IDB note.** Reuse the existing `KVStore` helper at `@/packages/shared/lib/indexeddb` rather than a new IDB wrapper. `persistence.ts` is a small typed facade over it.
