...
import { type Slide, parseAndRenderSlides, slideToPlainText } from '@/packages/shared/lib/slides';
...
export function SlidesPresentation({
  ...
  const { note: current } = useNote(noteId);
  ...
  const effectiveNoteName = noteName ?? current?.name;
  ...
  useEffect(() => {
    const processSlides = async () => {
      try {
        setLoading(true);
        setError(null);
        const parsedSlides = await parseAndRenderSlides(effectiveMarkdown, effectiveNoteName);
  ...
  }, [effectiveNoteName, effectiveMarkdown]);
      ...
      <div className={`flex items-center justify-center h-full ${className ?? ''}`}>
        <div className="max-w-md rounded-lg border bg-card px-6 py-8 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Add content to your note and use &quot;---&quot; to separate slides.
          </p>
        </div>
      </div>
    );
            ...
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  <span className="hidden sm:inline">{isGenerating ? 'Generating…' : 'Voice'}</span>
                </Button>
