...
export function SuggestionStripes({ stripes, pinnedChips, className }: SuggestionStripesProps) {
  ...
  /** Chip/board-word insert: append the chip text to the current editor text. */
  ...
  return (
    <div className={cn('flex flex-col gap-2.5', className)} ref={containerRef}>
      {/* Pinned word chips from the active board */}
        ...
        <div className="flex flex-wrap gap-2">
          {pinnedChips.map((chip, ci) => (
            <button
              ...
              className="flex h-11 animate-in fade-in slide-in-from-bottom-1 items-center gap-1.5 rounded-full border border-primary/30 bg-card px-5 text-base font-medium text-foreground transition-colors hover:border-primary/60 hover:bg-primary/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring motion-reduce:animate-none"
            >
      ...
      <div className="flex flex-col gap-2">
        {stripes.map((stripe, si) => {
          ...
          return (
            ...
            >
              <SourceMark source={stripe.source} />
...
function SourceMark({ source }: { source: Stripe['source'] }) {
  if (source === 'board') {
    return <Pin className="size-4 shrink-0 text-primary/60" aria-label="From board" />;
  }
  if (source === 'history') {
