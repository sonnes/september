import type { ModelSettings, WritingModelConfig } from '@september/core/rules/model-config';
...
export const WRITING_SERVICES = [
  ...
  },
  {
    value: 'openrouter',
    label: 'OpenRouter',
    description: 'Cloud service. Stronger AI Assistance. Free models are available.',
  },
  {
    value: 'none',
...
export const STEPS = [
  ...
  },
  {
    ...
    subtitle: 'Both answers are ready. Change one only if you want a different service.',
...
export function stepIndex(path: string, draft: Pick<OnboardingDraft, 'mode'>): number {
  return stepsFor(draft).findIndex(step => step.path === path);
}

export function nextStep(path: StepPath, draft: Pick<OnboardingDraft, 'mode'>): StepPath | null {
 ...
 */
export function isSetupDone(setup: Pick<OnboardingDraft, 'name' | 'mode'> | null): boolean {
