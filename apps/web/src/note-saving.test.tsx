...
const note = { id: 'note', space_id: 'space', user_id: 'user', name: 'Title', content: 'old', created_at: 1, updated_at: 1 };

describe('note durability', () => {
  it('starts saving on input, before a timer or unmount', async () => {
    save.mockResolvedValue(undefined);
    await act(async () => root.render(<NoteEditor note={note} spaceId="space" onRenamed={() => {}} />));
    await act(async () => type('Note', 'new words'));
    expect(save).toHaveBeenCalledWith(expect.objectContaining({ id: 'note', content: 'new words' }));
  });
  ...
  it('shows a failed save and guards closing until retry succeeds', async () => {
    ...
    expect((container.querySelector('[aria-label="Note"]') as HTMLTextAreaElement).value).toBe('keep these words');
  });
});
